...
## Mitigations, workarounds and roadmap

- Safety:
  - Run potentially untrusted code inside a separate process or container. Replace `__builtins__` with a restricted mapping for limited sandboxing (but this is non-trivial).
- Diagnostics:
  - Improve semantic analyzer to provide better messages, implement suggestion heuristics and show source snippets for runtime tracebacks.
- Features to add (high priority):
  - Lists / dictionaries, boolean literals & logical operators, argument arity checking, default/keyword args.
  - Module/import support and a small stdlib.
  - Better tests and CI to drive parity between transpiler and interpreter.
- Performance:
  - Add a simple optimizer (constant folding) and a few micro-optimizations in the interpreter; later consider an IR and native/LLVM backend.
- Usability:
  - Add REPL, nicer CLI flags, and optional sandboxing modes.